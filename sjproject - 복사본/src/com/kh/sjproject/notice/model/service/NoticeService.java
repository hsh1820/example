...
public class NoticeService {
	...
	public Notice selectNotice(int no) throws Exception {
		...
		// dao 여러번 호출하기 위해 미리 객체 생성
		
		// 공지사항 상세 조회 
		Notice notice = noticeDao.selectNotice(conn, no);
		
		// 공지사항 상세 조회 성공 시 조회수 증가
		if(notice!=null) {
			...
			if(result>0) {
				...
				rollback(conn);
				notice = null; // 조회 수 증가 실패 시 조회되지 않게 만듦.
			}
		}
	...
	public int insertNotice(Notice notice) throws Exception{
		...
		NoticeDao noticeDao = new NoticeDao();
		// 등록될 공지사항의 글 번호 생성
		// -> 공지사항 등록 성공 시 해당 글번호 상세 조회를 위해서 
		// 	+ 파일 업로드 시 게시판 현재 등록된 글 번호를 외래키로 사용하기 위해서
