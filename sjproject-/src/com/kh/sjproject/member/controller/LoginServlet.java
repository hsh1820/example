...
// ㄴ 서버 구동 시 web.xml에 현재 Servlet 클래스 명으로 <servlet>이 등록되고 매개변수에 작성된 url로 <servlet-mappin>이 작성된다.
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			// 비지니스 로직 처리를 위한 MemberService 클래스 생성하고 
			// login 기능을 수행 할 메소드 호출 + 결과 값 반환 받기 
			...
			if(loginMember!=null) {
				 ...
				 * session ID만 Cookie를 통해 Browser로 전달한다.
				 * 
				 * Cookie는 Server에 의해서 생성이 되지만 
				 * 관리는 Brower에서 진행한다. (Cookie 최대 크기는 5kb)
				 * */
				
				// C1) 아이디 저장 체크박스 값 가져오기 
				String save = request.getParameter("save");
				// System.out.println("save : " + save); 
				// -> 체크박스에 별도의 value가 없을 경우, 체크시 on를 반환하며 미 체크시에는 null 반환
				
				// C2) javax.servlet.http.Cookie 이용하여 쿠기 생성 
				Cookie cookie = new Cookie("saveId", memberId);
				
				// C3) 아이디 저장이 체크된 경우
				if(save!=null) {
					// 쿠기가 유지될 수 있는 유효기간 설정
					cookie.setMaxAge(60*60*24*7); // 7일로 
				}else {
					cookie.setMaxAge(0); // 쿠키 만료 
				}
				
				// C4) 쿠키가 사용될 수 있는 유효한 디렉토리(경로) 설정
				cookie.setPath("/"); // 해당 도메인 전역에서 사용이 가능하다.
				
				// C5) response 객체에 쿠키를 담아 클라이언트(브라우저) 전송
				response.addCookie(cookie);
				// => 이후 쿠키 관리는 브라우저가 맡아서 진행한다.
			...
			// --> 갱신이 되었다는 의미
			// --> 갱신이 되면 request, response 객체가 새로 생성된다.
