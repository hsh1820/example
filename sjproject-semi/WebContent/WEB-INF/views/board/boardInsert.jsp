...
<body>
	<div class="container">
		...
		<div class="container">
			...
			<form action="<%=request.getContextPath()%>/board/insert" method="post" 
				...
				<!-- multiple 속성 
					- input 요소에 사용 가능
					- input 요소 하나에 둘 이상의 값을 입력 할 수 있음을 명시
				 -->
					<input type="file" id="img1" 
						name="img1" onchange="LoadImg(this,1)"> 
						<!-- file 타입의 onchange : 파일 경로가 변경됐을 때  -->
						<input type="file" id="img2" multiple="multiple"
						name="img2" onchange="LoadImg(this,2)"> 
						<input type="file" id="img3" multiple="multiple"
						name="img3" onchange="LoadImg(this,3)"> 
						<input type="file" id="img4" multiple="multiple"
	...
	<script>
	    ...
	    function LoadImg(value, num) {
			...
			// value.files이라는 배열이 있고, value.files[0] 에 값이 있는지
			// --> 파일이 선택이 된 경우
			// ---> 경로만 읽어들이는 상태
	    	if(value.files && value.files[0]){
	    		var reader = new FileReader();
	    		// ---> 브라우저 위에 filereader객체가 올라옴
	    		reader.onload = function(e){
	    			switch(num){
	    			case 1 : $("#titleImg").prop("src",e.target.result);break;
	    			// reader.onload : 파일을 다 읽어들이면 파일 경로를 리턴함
	    			// => e.target : 읽어들인 파일의 경로가 들어있음
	    			// e : reader.onload 이벤트 
