...
<%
	String msg = (String)session.getAttribute("msg");
				// ㄴ object를 String으로 형변환, 다운캐스팅
	Member loginMember = (Member)session.getAttribute("loginMember");
	
	// C6) 쿠키 사용을 위한 변수 생성
	boolean save = false; // 아이디 저장 체크박스 값을 수정하기 위한 변수 
	String saveId = ""; // 쿠키에 저장된 saveId라는 키가 가지고 있는 값을 저장할 변수
	Cookie[] cookies = request.getCookies(); // 전달받은 쿠키 저장 // 요청데이터에서 얻어온 쿠키 (브라우저에 있는 쿠키를 얻어옴)
	
	// 서버 첫 시작시 request.getCookies()의 값이 null
	// -> if문으로 처리하지 않는 경우 페이지 로딩 시 NullPointerException이 발생된다.
	if(cookies!=null){
...
<body>
	<header class="blog-header">
		<div
							...
							<div class="modal-body">
								...
								>
									...
									<div class="checkbox mb-3">
										<label> 
											<input type="checkbox" id="save" name="save" <%= save? "checked" : "" %>> 아이디 저장
										</label>
									</div>
			...
			<%-- 로그인이 되어있지 않은 경우 end 지점 --%>
			<% } else {%>
			<%-- 로그인이 된 경우start 지점 --%>
			<div class="d-flex justify-content-end align-items-center"> 
				<%= loginMember.getMemberName() %>님 환영 합니다. &nbsp;
