...
public class BoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/list")) {
			...
			// 처음 게시판 목록으로 화면이 전환이 되면 1페이지가 보여야 한다
			...
			maxPage = (int)Math.ceil( ( (double)listCount / limit ) );
							// 올림 			50.0 / 5 =>10.0 을 올림하면 10.0 ->다시 int형변환 ->10
							//				51.0 / 5 =>10.2 을 올림하면 11.0 -> int형변환 ->11
			...
			try {
				...
				if(ServletFileUpload.isMultipartContent(request)) {
					...
					for(int i = originFiles.size()-1 ; i>=0 ; i--) {
						Attachment file = new Attachment(); // 파일정보 저장
						file.setFilePath(savePath); // 경로
					...
					if( (i == originFiles.size()-1) && multiRequest.getFilesystemName("img1") !=null ) { //마지막 인덱스를 0으로 하겠다??
						file.setFileLevel(0);
					}else {
						file.setFileLevel(1);
					}
		...
		// 이미지 다운로드용 Controller
			...
			try {
				int fileCount = new BoardService().getFileCount(fNo);
				int i = 1/fileCount;
				...
				if(file != null) {
					...
					int readBytes = 0;
					while((readBytes = bis.read()) != -1) { // 더이상 읽을게 없을 때 -1이 나온다.
