...
<%@page import="com.kh.searchfree.board.model.vo.Board"%>
<% 
...
<%-- 		<%@ include file="../common/nav.jsp"%> --%>
	...
	<script>
		...
		$(".boardImg").on("click",function(){
			var fNo = $(this).next().val();/* next() :형제관계의 다음 요소 == input태그 */
		...
		$("#addReply").on("click", function(){
			var writer;
			// 로그인 된사람만 댓글을 쓸 수 있도록 하고 팝업창을 띄우기 위하여 초기화 안함
			...
			<%} else{%>
				...
				$.ajax({
					...
					type : "post", // 문자셋 인코딩은 필터해둠
					data : {writer:writer, content:content, boardNo:boardNo},
					success : function(result){
						// success : forward/redirect 했을 때의 응답이 담겨서옴
						if(result>0){
							$("#replyContent").val("");
							selectRlist();
		...
		function selectRlist(){
			...
			$.ajax({
				...
				type : "post",
				dataType : "json",
				// 응답 데이터 형태 : json으로 전달받을 것이라는 의미
				data : {boardNo:boardNo},
				// 전달하려는 타입 == 요청한 데이터
				success : function(rList){
					// 성공하면 반환받을 결과물이 댓글 리스트
